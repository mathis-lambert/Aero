...
Verification: E2E `HistoryE2ETests` records visits from fixtures, searches, opens, deletes, clears and checks persistence across relaunch. Isolated `HistoryStoreTests` cover 1–7 and 9, which the UI cannot drive precisely (arbitrary search syntax, several profiles, backdated visits, damaged files). Failure mode 10 is blocked by the navigation policy, which only allows web, extension, `about` and `blob` URLs in pages; E2E `testWebsitesCannotOpenInternalPages` checks it.
